...
uxTour.prototype.start = function(tour) {
  ...
  if (document.getElementById("uxOverlay") === null) {
    ...
    this.overlay.style.cssText =
      "position: fixed; z-index: 999990; top: 0; left: 0; width: 100%; height: 100%;";

    this.highlight.id = "uxHighlight";
    this.highlight.style.cssText =
      "position: absolute; z-index: 999991; border-radius: " +
      (this.options.frame === "circle" ? "50" : "0") +
    ...
    this.tooltip.style.cssText =
      "position: absolute; z-index: 999991; width: 300px; min-width: 100%; padding: 8px; #FFF; opacity: 0; font-size: 16px; left: 50%; " +
      this.getPrefix().css +
    ...
    this.gotIt.style.cssText =
      "border: none; background: none; color: #FFF; padding: 0; outline: none; font-size: 16px; cursor: pointer;";
...
uxTour.prototype.showStep = function() {
  ...
  var step = this.tour.steps[this.currentStep],
    ...
    documentBody =
      browser.lowercase === "webkit" ? document.body : document.documentElement,
    padding,
    element,
    rect,
    marginTop,
  ...
  if (step === undefined) {
    ...
    if (rect.width > rect.height) {
      ...
      this.highlight.style.height =
        (this.options.frame === "circle" ? rect.width : rect.height) + "px";

      if (step.position === "fixed") {
        this.highlight.style.top =
          rect.top - padding + (marginTop + padding) + "px";
      } else {
        this.highlight.style.top =
          rect.top + window.pageYOffset + marginTop + "px";
      }

      this.highlight.style.left =
      ...
      this.highlight.style.width =
        (this.options.frame === "circle" ? rect.height : rect.width) + "px";
      ...
      this.highlight.style.left =
        rect.left + window.pageXOffset + marginLeft + "px";
...
uxTour.prototype.fadeIn = function() {
  this.boxShadow = !!navigator.userAgent.match(/Version\/[\d\.]+.*Safari/)
    ? "1900"
    : "9999"; // check for safari browser
  this.boxShadow =
    navigator.userAgent.indexOf("iPad") > -1 ? "893" : this.boxShadow; // check for iPad
  this.tooltip.style.opacity = 1;

  this.highlight.style.boxShadow =
    "0 0 0 " +
    this.boxShadow +
    "px rgba(" +
    this.options.color.r +
  ...
  if (direction === "top") {
    if (step.position === "fixed") {
      this.tooltip.style.top =
        elementRect.top - marginTop - tooltipRect.height - 50 + "px";
    } else {
      this.tooltip.style.top =
        elementRect.top +
        window.pageYOffset -
    ...
    if (step.position === "fixed") {
      ...
      this.tooltip.style.top =
        elementRect.top + window.pageYOffset + marginTop + 50 + "px";
    }
  } else {
    if (step.position === "fixed") {
      this.tooltip.style.top = elementRect.top + elementRect.height + 50 + "px";
...
uxTour.prototype.adjust = function() {
  ...
  if (
    document.getElementById("uxOverlay") !== null &&
